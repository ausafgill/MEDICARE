  ...
  Stream<List<OrderModel>> getActiveOrders() {
    return firestore
      ...
      return snapshot.docs.map((doc) {
        ...
        double price = 0;
        List<MedicineModel> medicalEquipments = [];
        for (var i = 0; i < order.medicines.length; i++) {
          if (order.medicines[i].pharmacyUid ==
              FirebaseAuth.instance.currentUser!.uid) {
            medicalEquipments.add(order.medicines[i]);
            price += (order.medicines[i].price *
                order.medicines[i].medicineQuantity);
          }
        }
        OrderModel newOrder = OrderModel(
            orderId: order.orderId,
            userId: order.userId,
            pharmaciesId: [FirebaseAuth.instance.currentUser!.uid],
            companyId: [],
            medicines: medicalEquipments,
            medicalEquipments: [],
